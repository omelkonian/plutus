...
Contracts write transactions using `submitTx {2c} (HasWriteTx s, AsContractError e) => LedgerTxConstraints -> Contract s e TxId`. The transactions produced that way are unbalanced and unsigned. When executing a `submitTx` instruction, the app platform forwards the transaction to the user's wallet to balance it (by adding appropriate public key inputs or outputs) and compute the gas cost of any scripts involved. The finished transaction is then signed by the signing process, and submitted to the wallet which sends it to the blockchain. `submitTx` returns the final transaction's ID once it has been submitted to the blockchain (or throws an error if the balancing or signing failed).
...
[source,haskell]
----
mkTx :: AddressMap -> PubKey -> LedgerTxConstraints
mkTx = undefined

spend :: (AsContractError e, HasBlockchainActions s) => Contract s e ()
spend = do -- <1>
    (ins, pk) <- collectRec
    void (submitTx (mkTx ins pk))
----
