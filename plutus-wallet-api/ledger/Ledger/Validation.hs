...
pubKeyOutput :: TxOut -> Maybe PubKey
pubKeyOutput TxOut{txOutType} = case txOutType of
    PayToPubKey pk -> Just pk
    _              -> Nothing
...
pubKeyOutputsAt pk p =
    let flt TxOut{txOutType, txOutValue} =
            case txOutType of
                PayToPubKey pk' | pk' == pk -> Just txOutValue
                _                           -> Nothing
    in mapMaybe flt (pendingTxOutputs p)
